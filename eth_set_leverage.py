...
if __name__ == "__main__":
    print(f"[{get_beijing_time()}] [INFO] 开始为所有账户设置ETH杠杆")
    ...
    for suffix in ACCOUNT_SUFFIXES:
        ...
        # 发送通知（可选）
        if success:
            print(f"[{get_beijing_time()}] [ACCOUNT-{suffix}] ETH杠杆设置成功")
        else:
            print(f"[{get_beijing_time()}] [ACCOUNT-{suffix}] ETH杠杆设置失败")
    
    print(f"[{get_beijing_time()}] [INFO] 所有账户ETH杠杆设置完成")
