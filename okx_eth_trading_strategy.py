...
name: OKX Eth 自动交易
定时规则
cron: 55 4,9,14,19,24,29,34,39,44,49,54,59 * * * *
"""
import os
import pandas as pd
import requests
...
def run_strategy(df):
    ...
    df['inRange1'] = (df['changePerc'] >= 0.8) & (df['changePerc'] <= 1.8)
    df['inRange2'] = ((df['high'] - df['low']) / df['low'] * 100) > 2
...
if __name__ == "__main__":
    ...
    # 获取K线数据
    result = market_api.get_candlesticks(instId=instId, bar="5m", limit="1")
    ...
    if signal:
        ...
        order_params = {
            ...
            "tpTriggerPx": str(round(entry_price * 1.015, 6)),
            "tpOrdPx": "-1",
            "slTriggerPx": str(round(entry_price * 0.981, 6)),
            "slOrdPx": "-1"
        }
