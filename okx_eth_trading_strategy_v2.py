...
# 振幅阈值参数
RANGE1_MIN = 1  # 振幅范围1最小值(0.8%)
RANGE1_MAX = 1.8  # 振幅范围1最大值(1.8%)
RANGE2_THRESHOLD = 1.9  # 振幅范围2阈值(2%)

# 交易执行参数
MARGIN = 5  # 保证金(USDT)
TAKE_PROFIT_PERCENT = 0.016  # 止盈比例改为1.6%
STOP_LOSS_PERCENT = 0.029  # 止损比例(2.6%)
...
def process_account_trading(account_suffix, signal, entry_price, amp_info):
    ...
    # 2. 计算合约数量
    size = round((MARGIN * LEVERAGE * 10) / entry_price, SizePoint)  # 乘以10是标准倍数
...
if __name__ == "__main__":
    print(f"[{get_beijing_time()}] [INFO] 开始ETH自动交易策略")
